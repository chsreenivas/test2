 ...
 * @author Cigniti
 */
public class ActiondriverM extends Base {
	...
	public static boolean click(By locator, String locatorName)
		...
		try {
			...
			if (flag) {
				Reporters.m_failureReport("Click "+locatorName, "Unable to click on :" + locatorName);
	...
	public static boolean isElementPresent(By by, String locatorName)
		...
		try {
			...
			if (flag) {
				...
				//Reporters.m_failureReport("check IsElementPresent", locatorName + " Element is not present on the page");
			}
		}
		return flag;
	...
	public static boolean type(By locator, String testdata, String locatorName)
		...
		boolean flag = false;
		try {
			driverM.findElement(locator).clear();
			driverM.findElement(locator).sendKeys(testdata);
			flag = true;
			return true;
		} catch (Exception e) {
			return false;
			...
			if (flag) {
				...
				Reporters.m_failureReport("Type ",			
	...
	public static boolean mouseover(By locator, String locatorName)
		...
		boolean flag = false;
		try {
			WebElement mo = driverM.findElement(locator);
			new Actions(driverM).moveToElement(mo).build().perform();
			...
			if (flag) {
				Reporters.m_failureReport("MouseOver",
	...
	public static boolean draggable(By source, int x, int y, String locatorName)
		...
		try {
			...
			WebElement dragitem = driverM.findElement(source);

			new Actions(driverM).dragAndDropBy(dragitem, x, y).build().perform();
			Thread.sleep(3000);
			flag = true;
		} catch (NoSuchElementException e) {
			...
			if (flag) {
				...
				Reporters.m_failureReport("Draggable",
	...
	public static void draganddrop(By source, By target, String locatorName)
		...
		boolean flag = false;
		try {
			WebElement from = driverM.findElement(source);
			WebElement to = driverM.findElement(target);
			new Actions(driverM).dragAndDrop(from, to).perform();
		} catch (Exception e) {
			flag = true;
		} finally {
			if (flag) {
				Reporters.m_failureReport("DragAndDrop",
	...
	public static boolean draganddropBy(WebElement source, WebElement target, int xOffset, int yOffset, String locatorName)
		...
		boolean flag = false;
		try {
			new Actions(driverM).clickAndHold(source).dragAndDropBy(target,xOffset,yOffset).release(target).build().perform();
			flag=true;
		} catch (Exception e) {
			return false;
		} finally {
	...
	public static void slider(By slider, int x,int y,String locatorName) throws Throwable {
		boolean flag = false;
		try {
			WebElement dragitem = driverM.findElement(slider);
			//	new Actions(driverM).dragAndDropBy(dragitem, 400, 1).build()	.perform();
			new Actions(driverM).dragAndDropBy(dragitem, x, y).build().perform();//150,0
			...
			if (flag) {
				Reporters.m_failureReport("Slider",
	...
	public static void rightclick(By by, String locatorName) throws Throwable {
		boolean flag = false;
		try {
			WebElement elementToRightClick = driverM.findElement(by);
			Actions clicker = new Actions(driverM);
			clicker.contextClick(elementToRightClick).perform();
			// driverM.findElement(by1).sendKeys(Keys.DOWN);
		} catch (Exception e) {
			flag = true;
		} finally {
			if (flag) {
				Reporters.m_failureReport("RightClick",
	...
	public static void waitForTitlePresent(By locator) throws Throwable {
		for (int i = 0; i < 200; i++) {
			if (driverM.findElements(locator).size()>0) {
				break;
			} else {
				Thread.sleep(50);
			}
	...
	public static boolean waitForElementPresent(By locator) throws Throwable {
		...
		try {
			for (int i = 0; i < 200; i++) {
				if (driverM.findElement(locator).isDisplayed()) {
					return true;
				} else {
					Thread.sleep(50);
				}
	...
	public static void selectBySendkeys(By locator, String value, String locatorName) throws Throwable {
		boolean flag = false;
		try {
			driverM.findElement(locator).sendKeys(value);
		} catch (Exception e) {
			flag = true;
		} finally {
			if (flag) {
				Reporters.m_failureReport("Select", value+ "is Not Select from the DropDown:" + locatorName);
				// throw new ElementNotFoundException("", "", "");
	...
	public static boolean selectByIndex(By locator, int index, String locatorName)
		...
		boolean flag = false;
		try {
			Select s = new Select(driverM.findElement(locator));
			...
			if (flag) {
				Reporters.m_failureReport("Select",
	...
	public static boolean selectByVisibleText(By locator, String visibletext,
		...
		try {
			...
			if (flag) {
				Reporters.m_failureReport("Select '"+visibletext+"'",visibletext+" is not Selected from the DropDown '" + locatorName+"'");
	...
	public static Boolean switchWindowByTitle(String windowTitle, int count)
		...
		boolean flag = false;
		try {
			Set<String> windowList = driverM.getWindowHandles();
			...
			/*	while (windowCount != count && second < timeout) {
				Thread.sleep(500);
				windowList = driverM.getWindowHandles();
			...
			for (int i = 0; i <=windowCount; i++) {
				...
				driverM.switchTo().window(array[count-1]);
				//if (driverM.getTitle().contains(windowTitle))
					return true;
			}
			return false;
		} catch (Exception e) {
			...
			if (flag) {
				Reporters.m_failureReport("SelectWindow",
	...
	public int getColumncount(By locator) throws Exception {
		...
		WebElement tr = driverM.findElement(locator);
	...
	public int getRowCount(By locator) throws Exception {
		...
		WebElement table = driverM.findElement(locator);
	...
	public boolean Alert() throws Throwable {
		...
		try {
			...
			if (presentFlag) {
				...
				Reporters.m_failureReport("Alert", "There was no alert to handle");
			}
		}
	...
	public static boolean isChecked(By locator, String locatorName) throws Throwable {
		boolean value = false;
		try {
			if (driverM.findElement(locator).isSelected()) {
				value = true;
			}
			...
			if (value) {
				...
				//Reporters.m_failureReport("IsChecked", locatorName
					//	+ " is not Select");
			}
		}
	}
	...
	public static Boolean isEnabled(By locator, String locatorName) throws Throwable {
		Boolean value = false;
		try {
			if (driverM.findElement(locator).isEnabled()) {
				value = true;
			}
			return value;
		} catch (NoSuchElementException e) {
			...
			if (value) {
				...
				Reporters.m_failureReport("IsEnabled", locatorName
						+ ":is not Enable");
			}
		}
	}
	...
	public static boolean isVisible(By locator, String locatorName) throws Throwable {
		...
		try {
			...
			value = driverM.findElement(locator).isDisplayed();
			return value;
		} catch (NoSuchElementException e) {
			return value;
		} finally {
			if (value) {
				...
				Reporters.m_failureReport("IsVisible", locatorName+ ":Element is Not Visible");
			}
		}
	}
	...
	public String getCssValue(By locator, String cssattribute,
		...
		if (isElementPresent(locator, "locatorName")) {
			value = driverM.findElement(locator).getCssValue(cssattribute);
		}
		return value;
	}
	...
	public static boolean verifyText(By by, String text,String locatorName) throws Throwable {
		...
		{
			Reporters.m_failureReport("VerifyText : "+text,text+" is not present in the location "+locatorName);
			flag=false;
		}
		return flag;
	}
	...
	public static void verifyText1(By by, String text,String locatorName) throws Throwable {
		...
		String a=driverM.findElement(by).getText();
	...
	public static boolean asserTitle(String title) throws Throwable {
		...
		try{
			...
			if(bRetValue){
				...
				Reporters.m_failureReport("asserTitle","Page title is not matched with "+title);
		}
		return bRetValue;
	}
	...
	public static boolean verifyTitle(String title) throws Throwable {
		...
		{
			Reporters.m_failureReport("VerifyTitle","Page title is not matched with "+title);
			return false;
		}
	...
	{  
		boolean flag;
		if((driverM.getPageSource()).contains(text))
		{
			flag=true;
		...
		{
			flag=false;
			//Reporters.m_failureReport("VerifyTextPresent "+text,text+" is not present on the page");
		}
		return flag;
	}
...

	{
		...
		if (driverM.getPageSource().contains("404")) {
			Reporters.m_failureReport("Verify404",text+" is present in the page");
	...
	public static String getAttribute(By by, String attribute,
		...
		if (isElementPresent(by, locatorName)) {
			value = driverM.findElement(by).getAttribute(attribute);
		}
		return value;
	}
	...
	public static boolean  isTextPresent(String text) throws Throwable {
		boolean value = driverM.getPageSource().contains(text);
		if (value) {
			//	Reporters.SuccessReport("IsTextPresent", "'" + text + "'"
			//			+ "is presented in the page");

		} else {
			Reporters.m_failureReport("IsTextPresent", "'" + text + "'"
					+ "is presented in the page");
			return false;
		}
		return value;
	...
	public static String getText(By locator, String locatorName)
		...
		String text = "";
		if (isElementPresent(locator, locatorName)) {
			text = driverM.findElement(locator).getText();
		}
		return text;
	}
	...
	public static void screenShot(String fileName) {
		File scrFile = ((TakesScreenshot) driverM)
	...
	public static void mouseHoverByJavaScript(By locator, String locatorName)
		...
		try {
			...
			JavascriptExecutor js = (JavascriptExecutor) driverM;
	...
	finally
	{
		...
		{
			Reporters.m_failureReport("SelectFrame","Frame with index "+index+"is not selected");
		}
		return flag;
	}
	}
	...
	try
	{
		driverM.switchTo().frame(idValue);
		flag=true;
	}
	catch(Exception e)
	{
	...
	finally
	{
		...
		{
			Reporters.m_failureReport("SelectFrame",
	...
	try
	{
		driverM.switchTo().frame(nameValue);
		flag=true;
	}
	catch(Exception e)
	{
	...
	try
	{
		driverM.switchTo().frame(driverM.findElement(locacator));
		flag=true;
	}
	catch(Exception e)
	{
	...
	public static boolean click1(WebElement locator,String locatorName ) throws Throwable
	{
		...
		try {
			...
			if (flag) {
				Reporters.m_failureReport("Click ", "Unable to click on :" + locatorName);
	...
	/**
	 * 
	 *  This method wait driverM until given driverM time.
	 *  
	 */
	public static WebDriverWait driverwait(){

		WebDriverWait wait = new WebDriverWait(driverM, 30);
		return wait;
	}
	...
	public static boolean waitForVisibilityOfElement(By by,String LocatorName) throws Throwable{
		...
		WebDriverWait wait = new WebDriverWait(driverM, 20);
		boolean flag=true;
		try {
			...
			if(flag){
				...
				//Reporters.m_failureReport("WaitForVisibility of Element "+LocatorName, LocatorName+" Element is not found");
			}
		}
		return flag;
	}
	...
	public static boolean waitForInVisibilityOfElement(By by, String LocatorName) throws Throwable{
		...
		try {
			...
			if(flag){
				...
				Reporters.m_failureReport("WaitForInVisibility of Element "+LocatorName, LocatorName+" Element is not inVisible");
			}
		}
	}
	...
	public static boolean waitForFrameToLoadAndSwitchToIt(By by, String LocatorName) throws Throwable{
		...
		try {
			...
			if(flag){
				...
				Reporters.m_failureReport("WaitForFrameToBeAvailable and Switch to It "+LocatorName, LocatorName+" is not found and unable to switch to it");
			}
		}
	}
	...
	public static boolean js_click(By by,String LocatorName) throws Throwable{
		boolean flag=true;
		try {
			WebElement elementToClick=driverM.findElement(by);
			((JavascriptExecutor) driverM).executeScript("arguments[0].click();", elementToClick);
			return true;
		} catch (Exception e) {
			flag=false;
			return false;
	...
	public static boolean js_TriggerOnClickEvent(String onClick_methodName,String LocatorName) throws Throwable{
		boolean flag=true;
		try {
			//((JavascriptExecutor) driverM).executeScript("addNewRecord();");
			((JavascriptExecutor) driverM).executeScript(""+onClick_methodName+"");
			return true;
		} catch (Exception e) {
			flag=false;
			return false;
	...
	public static boolean sendEnter(By by){
		try {
			//driverM.findElement(by).sendKeys(Keys.ENTER);
			new Actions(driverM).sendKeys(Keys.ENTER).perform();
			return true;
		} catch (Exception e) {
			return false;
		}
	...
	public static boolean isElementPresentInDOM(By by){
		try {
			driverM.manage().timeouts().implicitlyWait(2, TimeUnit.SECONDS);
			driverM.findElement(by);
			driverM.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
			return true;
		} catch (Exception e) {
			driverM.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
			return false;
	...
	public static boolean js_type(By by,String Text, String LocatorName) throws Throwable{
		boolean flag=true;
		try {
			WebElement location=driverM.findElement(by);
			//((JavascriptExecutor) driverM).executeScript("arguments[0].click();", elementToClick);
			((JavascriptExecutor)driverM).executeScript("arguments[0].value='"+Text+"'", location);
			return true;
		} catch (Exception e) {
			flag=false;
			return false;
			...
			{
				Reporters.m_failureReport("Type Data into "+LocatorName, "Able to Type Data into :" + LocatorName);
			}
		}
	}
	...
	public static String js_getText(By by, String LocatorName) throws Throwable{
		...
		if(isElementPresent(by, LocatorName)){
			labelNodeText = (String) ((JavascriptExecutor) driverM).executeScript("return arguments[0].innerHTML",driverM.findElement(by));
		}
