...
public class DirectEditOfRegistrant extends BusinessFunctions{
	public static int logIn=0;
	@Test(dataProvider = ("getData"),groups= {"EMT", "RunAll"},priority=0)
	public void createARegistrant(String prefix,String attendee,String first,String CustomerRegistrantID,String last,
		...
		if(clickRegistrant(attendee,first,last,Company))
		{
			...
			if(verifyRegistrantContactInformation(FullName, BadgeFirstName, Title, Phone, Mobile, AltPhone, Fax, Company, Address1, Address2, County, Region, City, CountryCode, Country, ZipCode, PersonalEmail, Email))
			{
	...
	public Object[][] getData() throws Exception {
		return Data_Provider.getTableArray("EMT_DirectEdit", "Key_AddRegistrant");  //returning data from "EMTReg" sheet and "Key_AddRegistrants" as a reference to read data
	}
	
	
	@Test(dataProvider = ("getEditData"),groups= {"EMT", "RunAll"},priority=1)
	public void EditRegistrant(String prefix,String attendee,String first,String CustomerRegistrantID,String last,
			String status,String AttendeeType,String SubAttendeeType,String WebURL,String Blog,String LinkedInURL,
			String FacebookURL,String TwitterAcnt,String IM,String AboutMe,String FullName,String BadgeFirstName,String Title,
			String Phone,String Mobile,String AltPhone,String Fax,String Company,String Address1,String Address2,String County,
			String Region,String City,String CountryCode,String Country,String ZipCode,String PersonalEmail,String Email,
		...
		if(click(By.xpath("//tr/td[text()='"+attendee+"']"),"Attendee"))
		{
			if(editRegistrantsInformation(prefix,attendee,first,CustomerRegistrantID,last,status,AttendeeType,SubAttendeeType))
			{
				Reporters.SuccessReport("Edit Registrant details", ""+first+" Registrant details got Edited");
			}
			else{
				Reporters.failureReport("Edit Registrant details", ""+first+" Registrant details is not Edited");
			}

			if(editRegistrantBio(WebURL, Blog, LinkedInURL, FacebookURL, TwitterAcnt,IM, AboutMe))
			{
				Reporters.SuccessReport("Edit Registrant information", ""+first+" Registrant information is Edited");
			}
			else
			{
				Reporters.failureReport("Edit Registrant information", ""+first+" Registrant information is not Edited");
			}

			if(editRegistrantContactInformation(FullName, BadgeFirstName, Title, Phone, Mobile, AltPhone, Fax, Company, Address1, Address2, County, Region, City, CountryCode, Country, ZipCode, PersonalEmail, Email))
			{
				Reporters.SuccessReport("Edit Registrant Contact information", ""+first+" Registrant Contact information is Edited");
			}
			else
			{
				Reporters.failureReport("Edit Registrant Contact information", ""+first+" Registrant Contact information is not Edited");
			}

			if(editRegistrantOptInInformation(OptInAttendeeSrch, OptInTicklerMsgs, OptInTwitterSesSelection, OptInSessAttendance, OptInRFIDAttendance))
			{
				Reporters.SuccessReport("Edit Registrant Opt In information", ""+first+" Registrant Opt In information is Edited");
			}
			else
			{
				Reporters.failureReport("Edit Registrant Opt In information", ""+first+" Registrant Opt In information is not Edited");
			}

			if(editRegistrantLoginInformation(LogInID, Pswd, ConfirmPassword))
			{
				Reporters.SuccessReport("Edit Registrant Login information", ""+first+" Registrant Login information is Edited");
			}
			else
			{
				Reporters.failureReport("Edit Registrant Login information", ""+first+" Registrant Login information is not Edited");
			}
			
			if(editRegistrantCategories(Industry, Product, JobRole, Division, Education, Certifications, CatRegion, Market, CompanySize, annualSales, RegPrimaryCode))
			{
				Reporters.SuccessReport("Edit Registrant categories", ""+first+" Registrant categories are Edited");
			}
			else
			{
				Reporters.failureReport("Edit Registrant categories", ""+first+" Registrant categories are not Edited");
			}

			if(editRegistrantAdditionalInfo(Demo1, Demo2, Demo3, Demo4, Demo5, Demo6, Demo7, Demo8, Demo9, Demo10, AccessFlag, ExecSummitFlag, HotelConfirmation, HotelSelection,
					MealTypeFlag, NickName, PromotionCode, RevRecFlag, StaffMealFlag, AddInfo4, AddInfo5, AddInfo1, AddInfo2, AddInfo3))
			{
				Reporters.SuccessReport("Edit Registrant additional information", ""+first+" Registrant additional information is Edited");
			}
			else
			{
				Reporters.failureReport("Edit Registrant additional information", ""+first+" Registrant additional information is not Edited");
			}
			...
			/*if(js_click(CommonOR.lnkDeleteThisRecord,"Delete This Registrant link")){
				...
				clickTabFromViewMore(CommonOR.lnkRegistrants, "Registrant Tab");

				search(attendee);

				if(verifyInTable("Registrants",attendee))
				{
					Reporters.failureReport("Registrant verification after delete", ""+attendee+" is still available in table");
				}
				else{
					Reporters.SuccessReport("Registrant verification after delete", ""+attendee+" is not available in table,hence successfully deleted");
				}
			}*/
			getEMTURL();
			emtLogout=true;
		...
		public Object[][] getEditData() throws Exception {
			...
			return Data_Provider.getTableArray("EMT_DirectEdit", "Key_EditRegistrant");  //returning data from "EMT_DirectEdit" sheet and "Key_EditRegistrant" as a reference to read data
		}
	}
