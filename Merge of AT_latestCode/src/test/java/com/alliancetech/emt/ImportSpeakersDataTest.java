...
public class ImportSpeakersDataTest extends BusinessFunctions{
	ArrayList<String>[] inputData ;
  @Test(groups={"EMT","RunAll"})
  public void importSpeakersCSVFileAndVerifyData() throws Throwable {
	  if(configProps.getProperty("HighlightElements").equalsIgnoreCase("true"))
	  {
		  driver.unregister(myListener);
	  }
	  inputData=new CSVFileIO().getAllRecordsAsArrayList(new File("TestData\\speaker.csv").getAbsolutePath());
	  if(getEMTURL())
		{
		...
		click(DataImports.lnkImportSpeakersData, "Import Speakers Data");

		if(uploadFile(new File("TestData\\speaker.csv").getAbsolutePath()))
		{
			Reporters.SuccessReport("Verify uploading of file", "Speaker csv file got successfully uploaded");
		}
		else
		{
			Reporters.failureReport("Verify uploading of file", "Speaker csv file failed to upload");
		}
		...
		if(verifyCSVData(inputData,"Login Id"))
		{
			Reporters.SuccessReport("Verification of Speaker details uploaded using csv file", "All the speaker information is valid");
		}
		else
		{
			Reporters.failureReport("Verification of Speaker details uploaded using csv file", "speaker information is not valid");
		}
		
		Thread.sleep(1000);
		
		if(deleteImportData(inputData,"Login Id"))
		{
