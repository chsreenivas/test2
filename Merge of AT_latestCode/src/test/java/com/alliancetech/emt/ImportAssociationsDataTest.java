...
public class ImportAssociationsDataTest extends BusinessFunctions{
	ArrayList<String>[] inputData ;
  @Test(groups={"EMT","RunAll"})
  public void importAssociationsCSVFileAndVerifyData() throws Throwable {
	 
	  if(configProps.getProperty("HighlightElements").equalsIgnoreCase("true"))
	  {
		  driver.unregister(myListener);
	  }
	  inputData=new CSVFileIO().getAllRecordsAsArrayList(new File("TestData\\association.csv").getAbsolutePath());
	  if(getEMTURL())
		{
		...
		if(uploadFile(new File("TestData\\association.csv").getAbsolutePath()))
		{
