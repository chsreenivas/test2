...
public class ImportSessionsDataTest extends BusinessFunctions{
	ArrayList<String>[] inputData ;
  @Test(groups={"EMT","RunAll"})
  public void importSessionsCSVFileAndVerifyData() throws Throwable {
	  if(configProps.getProperty("HighlightElements").equalsIgnoreCase("true"))
	  {
		  driver.unregister(myListener);
	  }
	  inputData=new CSVFileIO().getAllRecordsAsArrayList(new File("TestData\\SessionTestFile.csv").getAbsolutePath());
	  if(getEMTURL())
		{
		...
		if(uploadFile(new File("TestData\\SessionTestFile.csv").getAbsolutePath()))
		{
