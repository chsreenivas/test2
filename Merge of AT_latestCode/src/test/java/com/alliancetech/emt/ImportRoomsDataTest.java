...
public class ImportRoomsDataTest extends BusinessFunctions{
	ArrayList<String>[] inputData ;
	@Test(groups={"EMT","RunAll"})
	public void importRoomsCSVFileAndVerifyData() throws Throwable {
		if(configProps.getProperty("HighlightElements").equalsIgnoreCase("true"))
		  {
			  driver.unregister(myListener);
		  }
		inputData=new CSVFileIO().getAllRecordsAsArrayList(new File("TestData\\room.csv").getAbsolutePath());
		if(getEMTURL())
		{
		...
		click(DataImports.lnkImportRoomsData, "Import Rooms Data");

		if(uploadFile(new File("TestData\\room.csv").getAbsolutePath()))
		{
			Reporters.SuccessReport("Verify uploading of file", "Rooms csv file got successfully uploaded");
		}
		else
		{
			Reporters.failureReport("Verify uploading of file", "Rooms csv file failed to upload");
		}
		...
		if(verifyCSVData(inputData,"Name"))
		{
			Reporters.SuccessReport("Verification of Room details uploaded using csv file", "All the room information is valid");
		}
		else
		{
			Reporters.failureReport("Verification of Room details uploaded using csv file", "Room information is not valid");
		}
		
		Thread.sleep(1000);
		
		if(deleteImportData(inputData,"Name"))
		{
